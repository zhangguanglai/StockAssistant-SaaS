    ...
    const DEFAULT_STRATEGIES = {
        'trend_break': {
            ...
            description: '买入信号：①趋势(35分)MA20站稳+方向向上+高低点抬高；②MACD(12分)金叉/零上/柱增强；③均线形态(10分)MA5>MA10>MA20多头排列；④共振(15分)量价+均线+HL三看共振；⑤催化(10分)业绩/回购/涨停'
        },
    ...
    function resetForm(){addForm.value={ts_code:'',buy_price:'',buy_volume:'',buy_date:new Date().toISOString().slice(0,10),fee:0,note:'',reason:'',emotion:''};editingPosition.value=null;searchResults.value=[];buyConfirmedCheck.value=false}
    ...
    const watchStats=computed(()=>{
        const items=filteredWatchItems.value;let profit=0,loss=0,total=0,noData=0;
        items.forEach(w=>{const c=w.track_chg_pct;if(c==null||c===undefined){noData++;return;}total+=c;if(c>0)profit++;else if(c<0)loss++});
        return{profit,loss,noData,
            win_rate:(profit+loss)>0?Math.round(profit/(profit+loss)*100):0,
            avg_chg:items.length>0?Math.round(total/items.length*100)/100:0,
            count:(items.length - noData),
    ...
    async function loadWatchList(){
        ...
        try{
            ...
            const now=new Date();
            watchLastUpdate.value=`${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
