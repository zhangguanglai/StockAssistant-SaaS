    ...
    <div :class="['tab-content',activeTab==='screener'?'active':'']">
        ...
        <div v-if="Object.keys(strategyListSafe).length>0" class="strategy-selector">
            <div v-for="(meta,key) in strategyListSafe" :key="key"
                ...
                <!-- 展开详情（可折叠） -->
                <div class="strategy-details" v-if="currentStrategy===key">
                    <template v-if="!expandedDetails[key]">
                        <div class="strategy-tags strategy-collapsed">
                            <template v-if="key==='trend_break'">
                                <span class="tag"><span class="tag-icon">📌</span>趋势面(~37分) + MACD + 均线多头</span>
                                <span class="tag"><span class="tag-icon">🔥</span>板块(15) + 资金(10) + 共振(15) + 催化(10)</span>
                            </template>
                            <template v-else-if="key==='sector_leader'">
                                <span class="tag"><span class="tag-icon">📌</span>板块Top5 × 个股Top5 = 龙头或观察</span>
                                <span class="tag"><span class="tag-icon">⏱️</span>持股1-3日 / 止损亏损5%</span>
                            </template>
                            <template v-else-if="key==='oversold_bounce'">
                                <span class="tag"><span class="tag-icon">📌</span>趋势(35)：跌幅分级 + 止跌信号(长下影/放量阳/MACD金叉)</span>
                                <span class="tag"><span class="tag-icon">💰</span>板块(15/5) + 资金(15) + 加分(15) + 技术(15) + HL(-5~10)</span>
                            </template>
                        </div>
                    </template>
                    <template v-else>
                        <div class="strategy-tags">
                            <template v-if="key==='trend_break'">
                                <span class="tag"><span class="tag-icon">📌</span>买入信号：①趋势面(~37分)MA20位置(5)+方向(4)+HL结构(4)+MACD状态(0~12)+均线多头(0~10)；②板块(15分)相对超额收益+概念热度排名；③资金(10分)主力净流入+大单结构+持续性；④共振(15分)三看共振(全过15/二看9/一看5)；⑤催化(10分)业绩预告/回购/涨停/缩量回踩</span>
                            ...
                            <template v-else-if="key==='oversold_bounce'">
                                <span class="tag"><span class="tag-icon">📌</span>趋势(35分)：跌幅分级 &lt;-50=20 / -50~-40=17 / -40~-30=13 / -30~-25=10 / -25~-20=7</span>
                                <span class="tag"><span class="tag-icon">📌</span>止跌信号(最高15分)：长下影线+放量阳线+MACD金叉（三重=15/双=12/单=8）</span>
                                <span class="tag"><span class="tag-icon">🔥</span>板块效应(15分或5分)：命中热门概念Top15=15，未命中=5保底</span>
                                <span class="tag"><span class="tag-icon">💰</span>资金面(15分)：净流入&gt;0(+10) + 连续≥3天(+5) + 占比&gt;1%(+5)</span>
                                <span class="tag"><span class="tag-icon">🎁</span>加分项(15分)：市值区间(+10) + 业绩预增(+3) + 大额回购&gt;1亿(+2)</span>
                                <span class="tag"><span class="tag-icon">⚡</span>技术改善(15分)：MA5上穿MA20 + 量比≥2.0 + 阳线&gt;0.5%（三全=15/双=10/单=6）</span>
                                <span class="tag"><span class="tag-icon">📊</span>HL结构(-5~+10)：低点抬高=反弹信号(+10) / 继续创新低=-5 ⚠️</span>
                                <span class="tag"><span class="tag-icon">⚠️</span>过滤条件：非ST + 上市≥90天 + 近20日跌幅达标 + 市值范围 + 止跌信号≥1个</span>
                    ...
                    <button class="details-toggle-btn" @click.stop="toggleDetails(key)">
                        {{ expandedDetails[key] ? '▲ 收起规则详情' : '▼ 展开完整量化规则' }}
                    </button>
                </div>
                <!-- 执行摘要区（始终可见） -->
...
<div class="modal" style="max-width:700px">
    ...
    <!-- K-line Chart -->
    <div style="margin-bottom:16px">
        <div style="font-size:13px;font-weight:500;color:var(--text-secondary);margin-bottom:8px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px">
            <span>📈 K线分析</span>
            <!-- 指标切换 -->
            <div style="display:flex;gap:4px;flex-wrap:wrap">
                <button v-for="ind in ['VOL','MACD','KDJ','RSI','BOLL']" :key="ind"
                    :class="['btn','btn-xs',klineIndicator===ind?'btn-primary':'']"
                    @click="klineIndicator=ind;renderKlineChart()"
                    style="font-size:11px;padding:2px 8px">{{ind}}</button>
            </div>
        </div>
        <!-- 均线显隐控制 -->
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px;align-items:center">
            <span style="font-size:11px;color:var(--text-muted)">均线:</span>
            <label v-for="(vis,key) in maVisibility" :key="key" style="display:flex;align-items:center;gap:2px;cursor:pointer;font-size:11px;color:var(--text-secondary)">
                <input type="checkbox" v-model="maVisibility[key]" @change="renderKlineChart()" style="width:12px;height:12px">
